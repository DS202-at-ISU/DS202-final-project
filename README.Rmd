...
chunkref <- local({
  function(chunklabel) {
    sprintf('[@ref(%s)](#%s)', chunklabel, chunklabel )
  }  
})
...
In your write-up, make sure to refer to the figure. You can include a hyperlink to Figure `r chunkref("fig:code-chunk-name")`
[Figure 1](#fig:code-chunk-name) using the name of the code chunk (make sure, to give each code chunk a different name) by using `\@ref(fig:code-chunk-name)` in your write-up. \@ref(fig:code-chunk-name)
